...
import { USER_ROLES, MAINTENANCE_DEPT_ROLE_MATRIX } from "@/constants";

const MAINTENANCE_DEPT_ROLES = new Set(MAINTENANCE_DEPT_ROLE_MATRIX.map((r) => r.role)) as Set<string>;
...
const MAINTENANCE_DEPT_ROLE_OPTIONS = MAINTENANCE_DEPT_ROLE_MATRIX.map((r) => ({
  value: r.role,
  label: r.label,
}));
...
export default function MaintenanceEmployeesPage() {
  ...
  const items = useMemo(
    () => allItems.filter((u) => u.role && MAINTENANCE_DEPT_ROLES.has(u.role)),
    [allItems]
  );
  ...
  const closeCreateModal = useCallback(() => {
    ...
    if (searchParams.get("create") === "1") {
      router.replace("/maintenance/employees", { scroll: false });
    }
  }, [router, searchParams]);
  ...
  const [createForm, setCreateForm] = useState({
    ...
    password: "",
    phone: "",
    role: USER_ROLES.TECHNICIAN as string,
    branchId: "",
  });
    ...
    >
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Maintenance Department staff: branch manager, maintenance manager, technician, accountant, and maintenance.
            </p>
          </div>
          <Button
      ...
      <main className="mt-6">
        ...
        <Card className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50 px-4 py-4 sm:px-6">
            <CardTitle className="flex items-center gap-2 text-lg font-semibold text-slate-900">
              ...
              Maintenance staff ({filteredItems.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyState
                icon={Users}
                title="No maintenance employees"
                description="Create an employee to assign maintenance department roles (technician, maintenance manager, etc.)."
                      ...
                      <tr key={row._id} className="transition-colors hover:bg-slate-50/50">
                        ...
                        <td className="px-4 py-3">
                          <Badge variant="default">
                            {MAINTENANCE_DEPT_ROLE_OPTIONS.find((o) => o.value === row.role)?.label ?? formatRoleLabel(row.role ?? "")}
                          </Badge>
                        </td>
        ...
        <p className="mt-3 text-xs text-slate-500">
          Only users with maintenance department roles are listed. To assign or change roles, use{" "}
        ...
        <div className="relative overflow-hidden rounded-t-xl border-b border-slate-100 bg-gradient-to-r from-[#5a189a] to-[#7b2cbf] px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/90">Register a new maintenance department staff member.</p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <AppReactSelect
                ...
                options={MAINTENANCE_DEPT_ROLE_OPTIONS}
