...
const PERMISSION_LABELS: Record<string, string> = {
  [MAINTENANCE_DEPT_PERMISSIONS.EMPLOYEES_MANAGE]: "Manage employees",
  [MAINTENANCE_DEPT_PERMISSIONS.ROLES_VIEW]: "View staff & roles",
  [MAINTENANCE_DEPT_PERMISSIONS.TICKETS_VIEW]: "Maintenance Tickets",
  [MAINTENANCE_DEPT_PERMISSIONS.ASSETS_VIEW]: "Asset Tracking",
  [MAINTENANCE_DEPT_PERMISSIONS.REPORTS_VIEW]: "Maintenance Reports",
  [MAINTENANCE_DEPT_PERMISSIONS.PAYMENTS_VIEW]: "Income",
  [MAINTENANCE_DEPT_PERMISSIONS.EXPENSES_VIEW]: "Expenses",
  [MAINTENANCE_DEPT_PERMISSIONS.ACCOUNTING_VIEW]: "Accounting",
};
...
const PERMISSION_GROUPS = [
  {
    ...
    permissions: [MAINTENANCE_DEPT_PERMISSIONS.EMPLOYEES_MANAGE, MAINTENANCE_DEPT_PERMISSIONS.ROLES_VIEW],
  },
  {
    key: "ops",
    label: "Operations",
...
export default function MaintenanceRolesPage() {
  const currentUserRole = useAppSelector((state) => state.auth.user?.role ?? "");
  const knownPermissions = new Set(Object.values(MAINTENANCE_DEPT_PERMISSIONS));
      ...
      <header className="relative overflow-hidden border-b border-slate-100 bg-white">
        ...
        <div className="relative px-4 py-5 sm:px-6 sm:py-6 md:px-8">
          <div className="flex items-start gap-3 pl-3 sm:pl-4">
            ...
            <div>
              <span className="text-xs font-medium uppercase tracking-wider text-slate-500">Maintenance Department</span>
              ...
              <p className="mt-1 max-w-lg text-sm text-slate-600">
                Permissions by role for tickets, assets, reports, income, expenses, and accounting.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="px-4 py-6 sm:px-6 sm:py-8 md:px-8">
        <Card className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          <CardHeader>
            ...
            <CardDescription>
              Config: <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">MAINTENANCE_DEPT_PERMISSIONS</code>
              {" · "}
              <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">MAINTENANCE_DEPT_ROLE_PERMISSION_MATRIX</code>
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full min-w-[800px] text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80 text-left text-slate-600">
                    ...
                    {MAINTENANCE_DEPT_ROLE_MATRIX.map((roleDef) => (
                  ...
                  {PERMISSION_GROUPS.map((group) => (
                    <Fragment key={group.key}>
                      <tr className="bg-gradient-to-r from-[#5a189a]/5 to-[#ff9100]/5">
                        <td colSpan={2 + MAINTENANCE_DEPT_ROLE_MATRIX.length} className="py-2 pr-4 text-xs font-semibold uppercase tracking-wide text-[#5a189a]">
                          {group.label}
                        </td>
                      </tr>
                      {group.permissions
                          ...
                          <tr key={permission} className="transition-colors hover:bg-slate-50/50">
                            ...
                            {MAINTENANCE_DEPT_ROLE_MATRIX.map((roleDef) => (
                              <td key={roleDef.role} className="py-3 px-4">
                                <AccessCell
                                  allowed={MAINTENANCE_DEPT_ROLE_PERMISSION_MATRIX[roleDef.role]?.includes(permission) ?? false}
                                />
                              </td>
                            ))}
                          </tr>
