...
import {
  useEventBooking,
  useEventBookingPayments,
  useCreateEventBookingPayment,
} from "@/hooks/api";
import { Button, Input } from "@/components/ui";
import { AppReactSelect } from "@/components/ui/react-select";
import { ArrowLeft, Wallet, Plus, FileText, Receipt, RefreshCw, ReceiptText } from "lucide-react";
...
const TYPE_OPTIONS = [
  ...
  { value: "finalSettlement", label: "Final settlement" },
];
...
export default function EventBookingPaymentsPage() {
  const params = useParams<{ id: string }>();
  const bookingId = params?.id ?? "";

  const { data: bookingData, isLoading: bookingLoading, refetch: refetchBooking } = useEventBooking(bookingId);
  const { data: paymentsData, isLoading: paymentsLoading } = useEventBookingPayments(bookingId);
  const createMut = useCreateEventBookingPayment(bookingId);
  ...
  // Treat as chargeable if flag is set OR if amountToCharge is present (in case flag wasn't persisted)
  ...
  const derivedCustomerCharges = chargeableLineItems.reduce(
    (sum, row) => sum + Number(row.amountToCharge ?? row.amount ?? 0),
    0
  );
  // Use derived from expenseLineItems when it's larger (e.g. customerChargesTotal not yet saved or out of sync)
  const customerChargesTotal = Math.max(storedCustomerCharges, derivedCustomerCharges);
  const totalAmountDue = totalRevenue + customerChargesTotal;
  const depositPaid = Number(booking?.depositPaid ?? 0);
  const finalSettlementPaid = Number(booking?.finalSettlementPaid ?? 0);
  const computedOutstanding = Math.max(0, totalAmountDue - depositPaid - finalSettlementPaid);
  // When we derived charges from expense lines (e.g. stored was 0), use computed outstanding so Total to pay and Outstanding match
  const useComputedOutstanding =
    derivedCustomerCharges > 0 && derivedCustomerCharges >= storedCustomerCharges;
  const outstanding =
    useComputedOutstanding || booking?.outstandingAmount == null || !Number.isFinite(Number(booking.outstandingAmount))
      ? computedOutstanding
      : Number(booking.outstandingAmount);
  ...
  const typeLabel = (t: string) => (t === "deposit" ? "Deposit" : "Final settlement");
    ...
    <div className="min-h-screen bg-white font-sans" style={{ fontFamily: "Inter, sans-serif" }}>
        ...
        <div className="relative mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 max-w-xl text-sm text-slate-600">
                  ...
                  {booking?.title ?? booking?.clientName ?? "Event"} — record deposits and final settlement here.
                </p>
                <p className="mt-0.5 text-xs text-slate-500">
                  Totals and outstanding are from the saved event booking record. After saving customer charges on the Expenses page, click Refresh to load the updated record.
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link
                href={`/event-bookings/${bookingId}/expenses`}
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
        {isLoading ? (
          ...
          <>
            {/* Charges to customer (from event expenses) – show when any customer charges exist */}
            {customerChargesTotal > 0 && (
              <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50/80 p-4 shadow-sm">
                <p className="text-sm font-semibold text-amber-900">Charges to customer (expenses to pay)</p>
                <p className="mt-0.5 text-xs text-amber-800">
                  These amounts are added to the customer’s bill (e.g. damage, extra cleaning). Total: {fmt(customerChargesTotal)} — record a payment below to collect.
                </p>
                  ...
                  <ul className="mt-3 space-y-1">
                    {chargeableLineItems.map((row, i) => (
                      <li key={i} className="flex items-center justify-between text-sm text-amber-900">
                        <span>{String(row.category ?? "Charge").replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase())}</span>
                        <span className="font-medium">{fmt(Number(row.amountToCharge ?? row.amount ?? 0))}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="mt-2 text-sm text-amber-800">
                    Customer charges total: {fmt(customerChargesTotal)}. This is included in <strong>Total to pay</strong> and <strong>Outstanding</strong> below.
                  </p>
                )}
              </div>
            )}
            ...
            <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  Event: {fmt(totalRevenue)}
                  {customerChargesTotal > 0 ? ` + charges: ${fmt(customerChargesTotal)}` : " (no extra charges)"}
                </p>
                {customerChargesTotal === 0 && (
                  <Link
                    href={`/event-bookings/${bookingId}/expenses`}
                  ...
                  >
                    Add customer charges (e.g. damage) →
                  </Link>
                )}
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                ...
                <p className="mt-1 text-xl font-bold text-emerald-700">{fmt(depositPaid)}</p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Final settlement paid
                </p>
                <p className="mt-1 text-xl font-bold text-emerald-700">{fmt(finalSettlementPaid)}</p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Outstanding
                ...
                {customerChargesTotal > 0 && outstanding > 0 && (
                  <p className="mt-0.5 text-xs text-slate-500">
                    Includes event balance + {fmt(customerChargesTotal)} charges. Record payment below to settle.
                  </p>
                )}
              </div>
            </div>
            ...
            <section className="mb-8 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-5 shadow-sm">
              ...
              {outstanding > 0 && (
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  <p className="text-sm text-slate-600">
                    Outstanding is {fmt(outstanding)} (event balance{customerChargesTotal > 0 ? ` + ${fmt(customerChargesTotal)} customer charges` : ""}). Enter the amount the customer is paying.
                  </p>
                  <Button
                    type="button"
                    variant="outline"
              ...
              <form onSubmit={handleRecordPayment} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                ...
                <AppReactSelect
                  label="Type"
                  value={type}
                  onChange={(v) => setType((v as "deposit" | "finalSettlement") ?? "deposit")}
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-sm">
              ...
              {paymentsLoading ? (
                ...
                <div className="flex min-h-[120px] flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-xs text-slate-500">Record a deposit or final settlement above.</p>
                </div>
              ) : (
