...
const PERMISSION_LABELS: Record<string, string> = {
  [FINANCE_PERMISSIONS.EMPLOYEES_MANAGE]: "Manage employees",
  [FINANCE_PERMISSIONS.ROLES_VIEW]: "View staff & roles",
  [FINANCE_PERMISSIONS.INVOICES_VIEW]: "Invoices",
  [FINANCE_PERMISSIONS.CORPORATE_REPORTS_VIEW]: "Corporate Reports",
  [FINANCE_PERMISSIONS.FINANCIAL_REPORTS_VIEW]: "Financial Reports",
  [FINANCE_PERMISSIONS.PAYMENTS_VIEW]: "Payments",
  [FINANCE_PERMISSIONS.EXPENSES_VIEW]: "Expenses",
  [FINANCE_PERMISSIONS.DEPARTMENT_PL_VIEW]: "Department P&L",
};
...
const PERMISSION_GROUPS = [
  {
    ...
    permissions: [FINANCE_PERMISSIONS.EMPLOYEES_MANAGE, FINANCE_PERMISSIONS.ROLES_VIEW],
  },
  {
    key: "finance",
    label: "Finance",
...
export default function FinanceRolesPage() {
  const currentUserRole = useAppSelector((state) => state.auth.user?.role ?? "");
  const knownPermissions = new Set(Object.values(FINANCE_PERMISSIONS));
      ...
      <header className="relative overflow-hidden border-b border-slate-100 bg-white">
        ...
        <div className="relative px-4 py-5 sm:px-6 sm:py-6 md:px-8">
          <div className="flex items-start gap-3 pl-3 sm:pl-4">
            ...
            <div>
              <span className="text-xs font-medium uppercase tracking-wider text-slate-500">Finance</span>
              ...
              <p className="mt-1 max-w-lg text-sm text-slate-600">
                Permissions by role for invoices, corporate reports, and finance & accounting.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="px-4 py-6 sm:px-6 sm:py-8 md:px-8">
        <Card className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          <CardHeader>
            ...
            <CardDescription>
              Config: <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">FINANCE_PERMISSIONS</code>
              {" · "}
              <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">FINANCE_ROLE_PERMISSION_MATRIX</code>
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full min-w-[800px] text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80 text-left text-slate-600">
                    ...
                    {FINANCE_ROLE_MATRIX.map((roleDef) => (
                  ...
                  {PERMISSION_GROUPS.map((group) => (
                    <Fragment key={group.key}>
                      <tr className="bg-gradient-to-r from-[#5a189a]/5 to-[#ff9100]/5">
                        <td colSpan={2 + FINANCE_ROLE_MATRIX.length} className="py-2 pr-4 text-xs font-semibold uppercase tracking-wide text-[#5a189a]">
                          {group.label}
                        </td>
                      </tr>
                      {group.permissions
                          ...
                          <tr key={permission} className="transition-colors hover:bg-slate-50/50">
                            ...
                            {FINANCE_ROLE_MATRIX.map((roleDef) => (
                              <td key={roleDef.role} className="py-3 px-4">
                                <AccessCell
                                  allowed={FINANCE_ROLE_PERMISSION_MATRIX[roleDef.role]?.includes(permission) ?? false}
                                />
                              </td>
                            ))}
                          </tr>
