...
const PERMISSION_LABELS: Record<string, string> = {
  [CONFERENCE_PERMISSIONS.EMPLOYEES_MANAGE]: "Manage employees",
  [CONFERENCE_PERMISSIONS.ROLES_VIEW]: "View staff & roles",
  [CONFERENCE_PERMISSIONS.OVERVIEW_VIEW]: "Conference overview",
  [CONFERENCE_PERMISSIONS.EVENT_HALLS_VIEW]: "Event halls",
  [CONFERENCE_PERMISSIONS.EVENT_BOOKINGS_VIEW]: "Event bookings",
  [CONFERENCE_PERMISSIONS.EVENT_CALENDAR_VIEW]: "Event calendar",
  [CONFERENCE_PERMISSIONS.PROPOSALS_VIEW]: "Proposals pipeline",
  [CONFERENCE_PERMISSIONS.RESOURCES_VIEW]: "Resources",
  [CONFERENCE_PERMISSIONS.REPORTS_VIEW]: "Financial reporting",
  [CONFERENCE_PERMISSIONS.PAYMENTS_VIEW]: "Payments",
  [CONFERENCE_PERMISSIONS.EXPENSES_VIEW]: "Expenses",
  [CONFERENCE_PERMISSIONS.ACCOUNTING_VIEW]: "Accounting",
};
...
const PERMISSION_GROUPS = [
  {
    ...
    permissions: [
      CONFERENCE_PERMISSIONS.EMPLOYEES_MANAGE,
      CONFERENCE_PERMISSIONS.ROLES_VIEW,
    ],
  },
  {
    key: "events",
...
export default function ConferenceRolesPage() {
  const currentUserRole = useAppSelector((state) => state.auth.user?.role ?? "");
  const knownPermissions = new Set(Object.values(CONFERENCE_PERMISSIONS));
          ...
          <div className="flex items-start gap-3 pl-3 sm:pl-4">
            ...
            <div>
              ...
              <p className="mt-1 max-w-lg text-sm text-slate-600">
                Permissions by role for event managers, sales, coordinators, and accounting.
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="px-4 py-6 sm:px-6 sm:py-8 md:px-8">
        <Card className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          <CardHeader>
            ...
            <CardDescription>
              Config: <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">CONFERENCE_PERMISSIONS</code>
              {" · "}
              <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs">CONFERENCE_ROLE_PERMISSION_MATRIX</code>
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full min-w-[800px] text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80 text-left text-slate-600">
                    ...
                    {CONFERENCE_ROLE_MATRIX.map((roleDef) => (
                  ...
                  {PERMISSION_GROUPS.map((group) => (
                    <Fragment key={group.key}>
                      <tr className="bg-gradient-to-r from-[#5a189a]/5 to-[#ff9100]/5">
                        <td
                          colSpan={2 + CONFERENCE_ROLE_MATRIX.length}
                          ...
                          <tr key={permission} className="transition-colors hover:bg-slate-50/50">
                            ...
                            {CONFERENCE_ROLE_MATRIX.map((roleDef) => (
                              <td key={roleDef.role} className="py-3 px-4">
                                <AccessCell
                                  allowed={
                                    CONFERENCE_ROLE_PERMISSION_MATRIX[roleDef.role]?.includes(
                                      permission
                                    ) ?? false
                                  }
                                />
