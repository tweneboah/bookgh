...
import { usePlaygroundBooking } from "@/hooks/api";
...
const STATUS_LABELS: Record<string, string> = Object.fromEntries(
  Object.entries(PLAYGROUND_BOOKING_STATUS).map(([k, v]) => [
    v,
...
type Booking = {
  ...
  guestEmail?: string;
  guestPhone?: string;
  playgroundAreaId?: { _id?: string; name?: string; type?: string };
  bookingDate?: string;
  startTime?: string;
  endTime?: string;
...
export default function PlaygroundBookingDetailPage() {
  const params = useParams<{ id: string }>();
  const id = params?.id ?? "";
  const { data, isLoading } = usePlaygroundBooking(id);
        ...
        <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6 sm:py-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 flex-1 items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-0.5 truncate text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">
                  {b.guestName ?? "Playground booking"}
                </h1>
                <p className="mt-1 text-sm text-[#5a189a]/90">
                  {b.playgroundAreaId?.name ?? "—"} ·{" "}
                  {b.bookingDate
            ...
            <div className="flex flex-wrap items-center gap-2 sm:justify-end">
              <Link
                href={`/playground/bookings/${id}/payments`}
              ...
              <Link
                href={`/playground/bookings/${id}/expenses`}
          ...
          <article className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="space-y-4 px-5 py-4 sm:px-6 sm:py-5">
              <div>
                ...
                <p className="mt-1 font-semibold text-slate-900">
                  {(b.playgroundAreaId as { name?: string })?.name ?? "—"}
                </p>
              </div>
