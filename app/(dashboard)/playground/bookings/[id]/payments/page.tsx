...
import {
  ...
  usePlaygroundBookingPayments,
  useCreatePlaygroundBookingPayment,
} from "@/hooks/api";
...
export default function PlaygroundBookingPaymentsPage() {
  ...
  } = usePlaygroundBooking(bookingId);
  const { data: paymentsData, isLoading: paymentsLoading } =
    usePlaygroundBookingPayments(bookingId);
  const createMut = useCreatePlaygroundBookingPayment(bookingId);
  ...
  const derivedCustomerCharges = chargeableLineItems.reduce(
    (sum, row) =>
      sum + Number((row as any).amountToCharge ?? (row as any).amount ?? 0),
    0
  );
  const storedCustomerCharges = Number(booking?.customerChargesTotal ?? 0);
  const customerChargesTotal = Math.max(
    storedCustomerCharges,
    derivedCustomerCharges
  );
        ...
        <div className="relative mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 max-w-xl text-sm text-slate-600">
                  ...
                  {booking?.guestName ?? "Playground booking"} — record
                  payments here.
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link
                href={`/playground/bookings/${bookingId}/expenses`}
            ...
            {customerChargesTotal > 0 && (
              <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50/80 p-4 shadow-sm">
                <p className="text-sm font-semibold text-amber-900">
                  Charges to guest (expenses to pay)
                </p>
                <p className="mt-0.5 text-xs text-amber-800">
                  Total: {fmt(customerChargesTotal)} — record a payment below to
                  collect.
                </p>
                <Link
                  href={`/playground/bookings/${bookingId}/expenses`}
            ...
            <section className="mb-8 rounded-2xl border border-slate-200/80 bg-slate-50/50 p-5 shadow-sm">
              ...
              {outstanding > 0 && (
                <Button
                  ...
                  className="mb-4 rounded-lg border-amber-300 bg-amber-50 text-amber-800 hover:bg-amber-100"
                >
              ...
              >
                ...
                <AppReactSelect
                  ...
                  value={paymentMethod}
                  onChange={(v) =>
                    setPaymentMethod((v as string) ?? PAYMENT_METHOD.CASH)
                  }
