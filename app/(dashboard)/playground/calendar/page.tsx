...
import { usePlaygroundBookingCalendar } from "@/hooks/api";
...
export default function PlaygroundCalendarPage() {
  const now = new Date();
  ...
  const { data, isLoading } = usePlaygroundBookingCalendar(params);
  const playgroundAreas = data?.data?.playgroundAreas ?? [];
  ...
  const getBookingsForAreaDay = (areaId: string, day: Date) => {
    return (bookings as any[]).filter((b) => {
      const bAreaId = b.playgroundAreaId?._id ?? b.playgroundAreaId;
  ...
  const getMaintenanceForAreaDay = (areaId: string, day: Date) => {
    return (maintenance as any[]).filter((m) => {
      const mAreaId = m.playgroundAreaId?._id ?? m.playgroundAreaId;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">
            Playground Booking Calendar
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            View playground bookings and maintenance by date
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" asChild>
            <Link href="/playground/bookings">
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
          ...
          ) : playgroundAreas.length === 0 ? (
            <div className="py-20 text-center text-sm text-slate-400">
              No playground areas found. Add playground areas to see the
              calendar.
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-xs">
                ...
                <tbody>
                  {(playgroundAreas as any[]).map((area) => (
                    <tr key={area._id}>
                      ...
                      {days.map((day) => {
                        const dayBookings = getBookingsForAreaDay(
                          area._id,
                          day
                        );
                        ...
                        if (dayBookings.length > 0) {
                          ...
                          return (
                            ...
                            >
                              <Link
                                href={`/playground/bookings/${b._id}`}
                                className="block"
                              >
                                <div
                                  className={cn(
                                    "h-6 rounded border px-1 text-[10px] font-medium leading-6",
                                    colorClass
                                  )}
                                >
                                  {guestShort}
