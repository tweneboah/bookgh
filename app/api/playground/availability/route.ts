...
function generateSlots(
  ...
  for (
    let s = openMinutes;
    s + slotDurationMinutes <= closeMinutes;
...
function isDateInRecurrence(
  ...
  const start = new Date(
    maintenanceStart.getFullYear(),
    maintenanceStart.getMonth(),
    maintenanceStart.getDate()
  );
...
export const GET = withHandler(
  async (req, { auth }) => {
    ...
    const slotDurationMinutes = Math.min(
      120,
      Math.max(15, parseInt(sp.get("slotDurationMinutes") ?? "60", 10) || 60)
    );

    if (!dateParam) {
      throw new BadRequestError(
        "Query parameter 'date' (ISO date) is required"
      );
    }
    ...
    if (playgroundAreaIdParam) {
      if (!mongoose.Types.ObjectId.isValid(playgroundAreaIdParam)) {
        throw new BadRequestError("Invalid playgroundAreaId");
      }
      areaFilter._id = new mongoose.Types.ObjectId(playgroundAreaIdParam);
    }

    const areas = await PlaygroundArea.find(areaFilter as any).lean();
    if (areas.length === 0) {
    ...
    const [bookings, maintenanceList] = await Promise.all([
      PlaygroundBooking.find({
        tenantId: tenantOid,
        branchId: branchOid,
        playgroundAreaId: { $in: areaIds },
        bookingDate: { $gte: dayStart, $lte: dayEnd },
        status: { $in: activeStatuses },
      } as any)
        .select("playgroundAreaId startTime endTime numberOfGuests")
        .lean(),
      PlaygroundMaintenance.find({
        ...
        .select("playgroundAreaId scheduledDate startTime endTime recurrence")
        .lean(),
    ]);

    const availableSlots: Record<
      string,
      Array<{ start: string; end: string; availableCapacity: number }>
    > = {};

    for (const area of areas) {
      const a = area as any;
      const openMinutes = a.openingTime ? timeToMinutes(a.openingTime) : 0;
      const closeMinutes = a.closingTime
        ? timeToMinutes(a.closingTime)
        : 24 * 60;
      const capacity = a.capacity ?? 0;

      if (a.status !== PLAYGROUND_AREA_STATUS.OPEN) {
        availableSlots[String(a._id)] = [];
        continue;
      }

      const areaBookings = bookings.filter(
        (b: any) => String(b.playgroundAreaId) === String(a._id)
      );
      const areaMaintenance = maintenanceList.filter((m: any) => {
        if (String(m.playgroundAreaId) !== String(a._id)) return false;
        const sched = m.scheduledDate ? new Date(m.scheduledDate) : null;
        if (sched && dayStart <= sched && sched <= dayEnd) return true;
        if (m.recurrence && sched)
          return isDateInRecurrence(dayStart, sched, m.recurrence);
        return false;
      });
      ...
      for (const slot of allSlots) {
        ...
        const maintenanceBlocksSlot = areaMaintenance.some((m: any) => {
          const mStart = (m as any).startTime
            ? timeToMinutes((m as any).startTime)
            : 0;
          const mEnd = (m as any).endTime
            ? timeToMinutes((m as any).endTime)
            : 24 * 60;
    ...
    return successResponse({
      date: dateParam,
      playgroundAreas: areas.map((a: any) => ({
        _id: a._id,
        name: a.name,
        type: a.type,
        capacity: a.capacity,
