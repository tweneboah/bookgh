...
import { updateMaintenanceTicketSchema } from "@/validations/operations";
...
export const GET = withHandler(
  async (_req, { params, auth }) => {
    ...
    const doc = await MaintenanceTicket.findOne({
      _id: params.id,
      tenantId,
      branchId,
    } as Record<string, unknown>)
      .lean();
    if (!doc) throw new NotFoundError("Maintenance ticket");
...
export const PATCH = withHandler(
  async (req, { params, auth }) => {
    requireRoles(auth, [...MAINTENANCE_ROLES]);
    const { tenantId, branchId } = requireBranch(auth);
    const body = updateMaintenanceTicketSchema.parse(await req.json());
    const doc = await MaintenanceTicket.findOneAndUpdate(
...
export const DELETE = withHandler(
  async (_req, { params, auth }) => {
    requireRoles(auth, [...MAINTENANCE_ROLES]);
    const { tenantId, branchId } = requireBranch(auth);
    const doc = await MaintenanceTicket.findOneAndDelete({
      _id: params.id,
      tenantId,
      branchId,
    } as Record<string, unknown>);
