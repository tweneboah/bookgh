...
export const POST = withHandler(
  async (req, { auth }) => {
    requireRoles(auth, [...BAR_STOCK_ROLES]);
    ...
    for (const line of payload.lines) {
      ...
      const movement = await InventoryMovementModel.create({
        ...
        reason: `Bar stock count ${countId}`,
    ...
    await writeActivityLog(req, auth, {
      action: "BAR_STOCK_COUNT_COMPLETED",
      resource: "inventoryMovement",
      resourceId: results.length > 0 ? results[0].movementId : undefined,
      details: { stockCountId: countId, countedAt, notes: payload.notes, totals },
    });
