...
/** Generate time slots between open and close, e.g. every 30 or 60 minutes */
...
/** Check if a date falls on a recurring maintenance (simple weekly recurrence only for now) */
...
export const GET = withHandler(
  async (req, { auth }) => {
    ...
    if (!dateParam) {
      throw new BadRequestError("Query parameter 'date' (ISO date) is required");
    }
    ...
    const areaFilter: Record<string, unknown> = { tenantId: tenantOid, branchId: branchOid, isActive: true };
    if (poolAreaIdParam) {
      if (!mongoose.Types.ObjectId.isValid(poolAreaIdParam)) {
        throw new BadRequestError("Invalid poolAreaId");
      }
    ...
    for (const area of areas) {
      ...
      if (a.status !== POOL_AREA_STATUS.OPEN) {
      ...
      for (const slot of allSlots) {
        ...
        const maintenanceBlocksSlot = areaMaintenance.some((m: any) => {
          const mStart = (m as any).startTime ? timeToMinutes((m as any).startTime) : 0;
          const mEnd = (m as any).endTime ? timeToMinutes((m as any).endTime) : 24 * 60;
